...
def agemodify(self, subtr, appmin, edubon):
    ...
    print("Alright, then we need to remove a total of", subtr, "points from "
          "STR and CON and DEX as well as", appmin, "points from APP.")
    print("BUT: You get", edubon, "chances at a better EDU stat.")

    self.stats['APP'] -= min(self.stats['APP'], appmin)

    if subtr >= statsum:
        print("Sum of stats smaller than or equal", subtr, "all three stats "
              "set to zero.")
        self.stats['STR'] = 0
        self.stats['CON'] = 0
        self.stats['DEX'] = 0
    ...
    min1 = intInput(lower, upper, "Value",
                    "How many points do you want me to remove from STR? "
                    "(" + str(lower) + "-" + str(upper) + ") ")
    self.stats['STR'] -= min1
    subtr -= min1
    ...
    min2 = intInput(lower, upper, "Value",
                    "How many points from CON? ("
                    + str(lower) + "-" + str(upper) + ") ")
    self.stats['CON'] -= min2
    subtr -= min2
