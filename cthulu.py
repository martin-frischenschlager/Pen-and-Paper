...
class Character:
    ...
    # INFO
    info = {'occupation': '', 'age': '', 'birthday': '',
            'sex': '', 'residence': '', 'birthplace': ''}

    # ATTRIBUTES
    stats = {'STR': 0, 'DEX': 0, 'POW': 0, 'CON': 0, 'APP': 0, 'EDU': 0,
             'SIZ': 0, 'INT': 0, 'MOV': 0}
...
def intInput(a, b, inputname="Input", inputtext=""):
    ...
    while True:
        ...
        try:
            ...
            print(inputname, "must be between", a, "and", b)
        else:
            break
    return inp_int
...
def agemodify(self, subtr, appmin, edubon):
    ...
    print(f"Alright, then we need to remove a total of {subtr} points from "
          f"STR and CON and DEX as well as {appmin} points from APP.")
    print(f"BUT: You get {edubon} chances at a better EDU stat.")

    self.stats['APP'] -= min(self.stats['APP'], appmin)

    if subtr >= statsum:
        print(f"Sum of stats smaller than or equal {subtr}, all three stats "
              "set to zero.")
        self.stats['STR'] = 0
        self.stats['CON'] = 0
        self.stats['DEX'] = 0
    ...
    min1 = intInput(lower, upper, "Value", "How many points do you want me "
                    f"to remove from STR? ({str(lower)} - {str(upper)}) ")
    self.stats['STR'] -= min1
    subtr -= min1
    ...
    if subtr == (self.stats['CON'] + self.stats['DEX']):
        ...
        self.stats['CON'] = 0
        self.stats['DEX'] = 0
    lower = max(0, subtr + self.stats['STR'] + self.stats['CON'] + min1
                - statsum)
    upper = min(subtr, self.stats['DEX'])
    if lower == upper:
        min2 = upper
    else:
        min2 = intInput(lower, upper, "Value", "How many points from CON? "
                        f"({str(lower)} - {str(upper)}) ")
    self.stats['CON'] -= min2
    subtr -= min2
